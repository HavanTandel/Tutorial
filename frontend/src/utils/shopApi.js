...
export const shopApi = {
  ...
  // artworkDetails: (id) => api.get(`/artworks/${id}`).then(unwrap),

  // getCart: () => api.get("/orders/cart", withAuth()).then(unwrap),
  // addToCart: (artworkId, quantity = 1) =>
  //   api.post("/orders/cart/add", { artworkId, quantity }, withAuth()).then(unwrap),
  // updateCartItem: (artworkId, quantity) =>
  //   api.put(`/orders/cart/item/${artworkId}`, { quantity }, withAuth()).then(unwrap),
  // removeCartItem: (artworkId) =>
  //   api.delete(`/orders/cart/item/${artworkId}`, withAuth()).then(unwrap),
  // clearCart: () => api.delete("/orders/cart/clear", withAuth()).then(unwrap),

  // createCheckout: (payload) => api.post("/orders/checkout/create", payload, withAuth()).then(unwrap),
  // verifyCheckout: (payload) => api.post("/orders/checkout/verify", payload, withAuth()).then(unwrap),

  // myOrders: () => api.get("/orders/my-orders", withAuth()).then(unwrap),
  // trackOrder: (id) => api.get(`/orders/my-orders/${id}/tracking`, withAuth()).then(unwrap),
  // cancelOrder: (id) => api.put(`/orders/cancel/${id}`, {}, withAuth()).then(unwrap),
